...
<div class="section-card">
  ...
  <div class="alert-box info"><span>Chaque cuisine (viennoiserie, libanais, glacier…) reçoit automatiquement les ordres de fabrication des familles qui lui sont assignées dans <strong>Famille par production</strong>. Créez un compte utilisateur « production » rattaché à une cuisine depuis <strong>Utilisateurs</strong>.</span></div>
  <div class="table-wrap">
    <table>
      <thead><tr><th>Nom</th><th>Description</th><th>Familles</th><th>Comptes</th><th>Statut</th><th>Actions</th></tr></thead>
...
async function saveCuisine() {
  ...
  if (r.ok) { LABO.closeModal('modal-cuisine'); LABO.toast('Enregistré ✓'); loadCuisines(); }
  else LABO.toast(r.error || 'Erreur', 'error');
}
async function delCuisine(id) {
  if (!confirm('Supprimer cette cuisine ? Les familles et comptes rattachés seront simplement désassignés.')) return;
  const r = await LABO.api('cuisine_delete', { id });
  if (r.ok) { LABO.toast('Supprimé'); loadCuisines(); } else LABO.toast(r.error || 'Erreur', 'error');
}
